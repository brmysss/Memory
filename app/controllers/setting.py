...
class SettingController(CRUDBase[Setting, SettingCreate, SettingUpdate]):
    ...
    async def update(
        ...
        logger.debug(f"接收到的obj_in类型: {type(obj_in)}")
        logger.debug(f"接收到的obj_in内容: {obj_in}")
        ...
        logger.debug(f"处理后的obj_dict: {obj_dict}")
        
        obj = await self.get(id=id)
        logger.debug(f"更新前的obj.meta: {obj.meta}")
        ...
        for key, value in obj_dict.items():
            if key in ['meta', 'general', 'content', 'storage', 'database'] and value is not None:
                ...
                if isinstance(current_value, dict) and isinstance(value, dict):
                    ...
                    logger.debug(f"合并字段 {key}: {current_value} + {value} = {merged_value}")
                    setattr(obj, key, merged_value)
                else:
                    logger.debug(f"直接设置字段 {key} = {value}")
                    setattr(obj, key, value)
            else:
                logger.debug(f"设置字段 {key} = {value}")
                setattr(obj, key, value)
        
        logger.debug(f"更新后的obj.meta: {obj.meta}")
        await obj.save()
        logger.debug(f"保存后的obj.meta: {obj.meta}")
        return obj
